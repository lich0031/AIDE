...
from datasetchaos_proposed import chaos_seg, Compose, Resize, RandomRotate, RandomHorizontallyFlip, ToTensor, Normalize
...
def parse_args():
    ...
    parser.add_argument('--checkpoint', default='checkpoint_chaos_proposed30cases1label/')
    parser.add_argument('--history', default='history_chaos_proposed30cases1label')
    parser.add_argument('--cudnn', default=0, type=int, help='cudnn')
    parser.add_argument('--repetition', default=200, type=int, help='...')
...
def Train(train_root, train_csv, test_csv, traincase_csv, testcase_csv, labelcase_csv, tempmaskfolder):
    ...
    train_dataset = chaos_seg(root=train_root, csv_file=train_csv, tempmaskfolder=tempmaskfolder, transform=train_aug)
    test_dataset = chaos_seg(root=train_root, csv_file=test_csv, tempmaskfolder=tempmaskfolder, transform=test_aug)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for batch_idx, (inphase, outphase, augset, targets, targets1, targets2) in \
            ...
            for aug_idx in range(augset['augno'][0]):
                ...
                augimgout = augset['imgmodal2{}'.format(aug_idx + 1)].to(device)
        ...
        if traincasediceavgtemp > besttraincasedice:
            ...
            savecheckname = os.path.join(args.checkpoint, params2_name.split('.pkl')[0] +
                                         '_besttraincasedicde.' + params2_name.split('.')[-1])
            torch.save(state, savecheckname)
        ...
        if (epoch + 1) <= args.warmup_epoch or (epoch + 1) % 10 == 0:
                ...
                if caseidx not in label_cases:
                    ...
                    for imgidx in range(len(caseinphaseimg)):
                        ...
                        save_name = os.path.join(save_folder,
                                                 casemask[imgidx].split('/')[-1].split('.')[0] + '_net1.png')
            ...
            logging.info('Mask {} modify for net1'.format([train_cases[i] for i in selectedidxs]))
            ...
            for selectedidx in selectedidxs:
                ...
                if caseidx not in label_cases:
                    ...
                    for imgidx in range(len(caseinphaseimg)):
                        ...
                        makefolder(save_folder)
                        save_name = os.path.join(save_folder,
                                                 casemask[imgidx].split('/')[-1].split('.')[0] + '_net2.png')
                        save_data = generatedmask2[selectedidx][:, :, imgidx]
                        output_pil = save_data * 63
                        output_pil = Image.fromarray(output_pil.astype(np.uint8), 'L')
                        output_pil.save(save_name)
            logging.info('Mask {} modify for net2'.format([train_cases[i] for i in selectedidxs]))
...
if __name__ == "__main__":
    ...
    tempmaskfolder = 'generated_masks_1casepretrain'
    makefolder(os.path.join(train_root, tempmaskfolder))
    tempmaskfolder = 'generated_masks_1casepretrain/besttraincasedice_{}_{}'.format(args.model_name, args.repetition)
